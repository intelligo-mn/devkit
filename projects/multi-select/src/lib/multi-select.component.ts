...
export class MultiSelectComponent implements DoCheck, OnChanges {
  ...
  confirmed: BasicSelectData;

  sourceDiffer: any;
  destinationDiffer: any;
  @Input() sourceLoading: boolean;
  @Input() destinationLoading: boolean;
  ...
  dragEnd(list: BasicSelectData = null): boolean {
    if (list) {
  ...
  drag(event: DragEvent, item: any, list: BasicSelectData) {
  ...
  private makeUnavailable(source: BasicSelectData, item: any) {
  ...
  clickItem(list: Array<any>, item: any, isConfirm: boolean) {
    const pk = list.filter((e: any) => {
      return Object.is(e, item);
    });
    if (pk.length > 0) {
  ...
  selectAll(source: BasicSelectData, isConfirm: boolean) {
    source.pick.length = 0;
  ...
  isAllSelected(source: BasicSelectData): boolean {
  ...
  isAnySelected(source: BasicSelectData): boolean {
  ...
  private unpick(source: BasicSelectData) {
  ...
  onFilter(source: BasicSelectData) {
